...
export const sendPasswordResetEmail = async (email, otp) => {
  try {
    const data = await resend.emails.send({
      from: process.env.EMAIL_FROM || 'todo-notice@cshrpro.com',
      to: email,
      subject: '【Dodo Todo】パスワードリセット',
      text: `Dodo Todo パスワードリセット

このメールは、パスワードリセットのリクエストを受け付けました。

確認コード：${otp}

このコードは15分後に有効期限が切れます。
...
import { getProUserEmails } from './subscription.js';

export const sendDailySummaryEmail = async (stats) => {
  const proEmails = await getProUserEmails();
  const recipients = proEmails.length > 0 ? proEmails : ['chen.qiangqiang@outlook.com', 'seth.chen@outlook.com'];
  const {
    createdCount,
    delayedCount,
    overdueCount,
...
const formatWeekRange = (weekStart, weekEnd) => {
  const start = new Date(weekStart).toLocaleDateString('ja-JP', {
    month: 'long', day: 'numeric'
  });
  const end = new Date(weekEnd).toLocaleDateString('ja-JP', {
    month: 'long', day: 'numeric'
  });
  return `${start} ～ ${end}`;
};

const createWeeklyTodoItemText = (index, todo, type) => {
  let text = `\n【${index}】📧 ${todo.email}${todo.name ? ` (${todo.name})` : ''}\n`;
  text += `　　📝 タイトル: ${todo.title}\n`;
  if (todo.description) {
    text += `　　📖 説明文: ${todo.description}\n`;
  }
...
const createWeeklyTodoItemHtml = (index, todo, type, color) => {
  let statusChange = '';
  let deadlineLine = '';
  ...
  if (type === 'created') {
    ...
    statusChange = `<span style="color: #6b46c1; font-weight: bold;">pending（未完了）</span>`;
    deadlineLine = `<p style="margin: 4px 0; color: #6b7280; font-size: 13px;">⏰ 期限: ${formatDateShort(todo.deadline)}</p>`;
  } else if (type === 'delayed') {
    statusChange = `<span style="color: #6b46c1;">${todo.oldStatus}</span> → <span style="color: #ea580c; font-weight: bold;">delayed（+${todo.delayDays}日）</span>`;
    deadlineLine = `<p style="margin: 4px 0; color: #6b7280; font-size: 13px;">⏰ 新しい期限: ${formatDateShort(todo.newDeadline)}</p>`;
...
export const sendWeeklySummaryEmail = async (stats) => {
  const proEmails = await getProUserEmails();
  const recipients = proEmails.length > 0 ? proEmails : ['chen.qiangqiang@outlook.com', 'seth.chen@outlook.com'];
  const {
    createdCount,
  ...
  let emailContent = `
【Dodo Todo】週間サマリー (${weekRange})

━━━━━━━━━━━━━━━
📊 今週の集計
━━━━━━━━━━━━━━━
📝 作成: ${createdCount}件 | ⏰ 延期: ${delayedCount}件 | ⚠️ 期限切れ: ${overdueCount}件
✅ 完了: ${completedCount}件 | ❌ 放棄: ${abandonedCount}件 | 🔔 未完了: ${pendingCount}件
📈 完了率: ${completionRate}%
`;

  if (todosData.pending.length > 0) {
    emailContent += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n🔔 未完了のTodo（${todosData.pending.length}件）\n━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
    todosData.pending.forEach((todo, i) => {
      emailContent += createWeeklyTodoItemText(i + 1, todo, 'pending');
    });
  }

  if (todosData.created.length > 0) {
    emailContent += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n📋 今週作成されたTodo（${todosData.created.length}件）\n━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
    todosData.created.forEach((todo, i) => {
      emailContent += createWeeklyTodoItemText(i + 1, todo, 'created');
    });
  }

  if (todosData.delayed.length > 0) {
    emailContent += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n⏰ 今週延期されたTodo（${todosData.delayed.length}件）\n━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
    todosData.delayed.forEach((todo, i) => {
      emailContent += createWeeklyTodoItemText(i + 1, todo, 'delayed');
    });
  }

  if (todosData.overdue.length > 0) {
    emailContent += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n⚠️ 今週期限切れになったTodo（${todosData.overdue.length}件）\n━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
    todosData.overdue.forEach((todo, i) => {
      emailContent += createWeeklyTodoItemText(i + 1, todo, 'overdue');
    });
  }

  if (todosData.completed.length > 0) {
    emailContent += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n✅ 今週完了したTodo（${todosData.completed.length}件）\n━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
    todosData.completed.forEach((todo, i) => {
      emailContent += createWeeklyTodoItemText(i + 1, todo, 'completed');
    });
  }

  if (todosData.abandoned.length > 0) {
    emailContent += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n❌ 今週放棄したTodo（${todosData.abandoned.length}件）\n━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
    todosData.abandoned.forEach((todo, i) => {
      emailContent += createWeeklyTodoItemText(i + 1, todo, 'abandoned');
    });
  }
  ...
  let emailHtml = `
<div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  <h2 style="color: #6b46c1; margin-bottom: 8px;">【Dodo Todo】週間サマリー</h2>
  <p style="color: #6b7280; font-size: 14px; margin-bottom: 24px;">${weekRange}</p>

  <div style="background-color: #f9fafb; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
    <h3 style="color: #374151; font-size: 14px; margin: 0 0 12px; font-weight: bold;">📊 今週の集計</h3>
    <div style="display: flex; flex-wrap: wrap; gap: 12px;">
      ...
      <div style="flex: 1; min-width: 100px;">
        <span style="color: #6b7280; font-size: 12px;">🔔 未完了</span>
        <p style="margin: 2px 0 0; font-size: 18px; font-weight: bold; color: #6b46c1;">${pendingCount}件</p>
      </div>
      <div style="flex: 1; min-width: 100px;">
        <span style="color: #6b7280; font-size: 12px;">📈 完了率</span>
        <p style="margin: 2px 0 0; font-size: 18px; font-weight: bold; color: #16a34a;">${completionRate}%</p>
...
`;
  ...
  <div style="margin-bottom: 24px;">
    ...
    ${todosData.pending.map((todo, i) => createWeeklyTodoItemHtml(i + 1, todo, 'pending', '#6b46c1')).join('')}
  </div>
`;
  }
  ...
  <div style="margin-bottom: 24px;">
    ...
    ${todosData.created.map((todo, i) => createWeeklyTodoItemHtml(i + 1, todo, 'created', '#6b46c1')).join('')}
  </div>
`;
  }
  ...
  <div style="margin-bottom: 24px;">
    ...
    ${todosData.delayed.map((todo, i) => createWeeklyTodoItemHtml(i + 1, todo, 'delayed', '#ea580c')).join('')}
  </div>
`;
  }
  ...
  <div style="margin-bottom: 24px;">
    ...
    ${todosData.overdue.map((todo, i) => createWeeklyTodoItemHtml(i + 1, todo, 'overdue', '#9333ea')).join('')}
  </div>
`;
  }
  ...
  <div style="margin-bottom: 24px;">
    ...
    ${todosData.completed.map((todo, i) => createWeeklyTodoItemHtml(i + 1, todo, 'completed', '#16a34a')).join('')}
  </div>
`;
  }
  ...
  <div style="margin-bottom: 24px;">
    ...
    ${todosData.abandoned.map((todo, i) => createWeeklyTodoItemHtml(i + 1, todo, 'abandoned', '#dc2626')).join('')}
  </div>
`;
  }
...
`;
  ...
  try {
    const data = await resend.emails.send({
      from: process.env.EMAIL_FROM || 'todo-notice@cshrpro.com',
      to: recipients,
      subject: `【Dodo Todo】週間サマリー (${weekRange})`,
      text: emailContent,
      html: emailHtml
    });
